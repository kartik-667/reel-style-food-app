...
//               return {...ele, likeCount:ele.likeCount+1 }
//             }else{
//               return ele
//             }
//           })
...
//                 <FaRegHeart size={22} />
//                 <span  className="text-[14px]">Like {video.likeCount || 0}</span>
//               </button>
  ...
  const handleLike = async (video) => {
    try {
      const result = await axios.post(
        "http://localhost:5555/api/food/like",
        { foodID: video._id },
        { withCredentials: true }
      );
  ...
  const handleSave = async (video) => {
    try {
      const response = await axios.post(
        "http://localhost:5555/api/food/save",
        { foodID: video._id },
        { withCredentials: true }
      );
      console.log(response);
